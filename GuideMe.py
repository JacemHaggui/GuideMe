...
# Personal libraries
import HSV as HSV
import GuidingLogic as gd
from Play_Sound import *
from camera_setup import setup_camera
import Haptic_Luna_Setup as hl
from FPS import *
import config
...
piCam, dispW, dispH = setup_camera()


# Start TF-Luna thread
tfluna_thread = threading.Thread(target = hl.read_and_process_data, daemon=True)
tfluna_thread.start()
...
while True:
    ...
    # Capture frame from Raspberry Camera
    ...
    filtered_contours = [contour for contour in contours if cv.contourArea(contour) > config.min_area_threshold_redlines]
    ...
    if len(filtered_contours) >= 2:
        ...
        filtered_feet_contours = [contour for contour in contours if cv.contourArea(contour) > config.min_area_threshold_feet]
        ...
        if len(filtered_feet_contours) >= 2: # we likely have both feet
            ...
            for contour in sorted_feet_contours_area:
                ...
                if M["m00"] != 0:
                    ...
                    cv.circle(frame, (cx, cy), 5, (0, 0, 255), -1)  # Draw center for visualization
            ...
            if(shortest_distance < config.distance_to_red_threshold):
                play_sound(f"/home/jacemhagui/GuideMe/guideme/Audio/Sound Bank/Attention, {foot_flag}.mp3")
            ...
            if M["m00"] != 0:
                ...
                cy = int(M["m01"] / M["m00"])
                center = (cx, cy)
                cv.circle(frame, (cx, cy), 5, (0, 255, 0), -1)  # Draw center for visualization
            ...
            # Display the distances for left and right feet
            #cv.putText(frame, f"Foot distance to left line: {distance_to_leftline:.2f}", (dispW // 4, dispH // 2 + 30), cv.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
            #cv.putText(frame, f"Foot distance to right line: {distance_to_rightline:.2f}", (dispW // 4, dispH // 2 + 60), cv.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
        else:
            cv.putText(frame, "Feet not detected", (dispW // 4, dispH // 2), cv.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 0), 2)
            play_sound("/home/jacemhagui/GuideMe/guideme/Audio/Sound Bank/Feet_not_detected.mp3")
    else:
        cv.putText(frame, "Line not detected", (dispW // 4, dispH // 2), cv.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
        play_sound("/home/jacemhagui/GuideMe/guideme/Audio/Sound Bank/Line_not_detected.mp3")
